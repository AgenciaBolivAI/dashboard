-- =====================================================================
-- BolivAI — Step 7: video meeting room per reservation
-- =====================================================================
-- Apply AFTER schema-step6-customer-notify.sql.
--
-- Adds meeting_provider / meeting_url / meeting_room_name columns to
-- reservations. The notify workflow creates a Daily.co room scheduled to
-- auto-expire at end_at and writes the join URL back here. Both the
-- customer email and the dashboard calendar render the URL.
--
-- Also includes the meeting_url in the notify trigger payload so the
-- workflow can decide whether to skip room creation (already exists)
-- when an event re-fires.
...
comment on column reservations.meeting_provider is
  'Video meeting backend identifier (e.g. "daily").';
comment on column reservations.meeting_url is
  'Direct join URL for the scheduled meeting. Auto-expires at end_at.';
comment on column reservations.meeting_room_name is
  'Provider-side room identifier so the room can be deleted/extended later.';
