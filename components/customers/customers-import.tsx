...
function guessColumn(field: string, headers: string[]): string {
  ...
  const aliases: Record<string, string[]> = {
    name: ["name", "fullname", "nombre", "contact", "cliente", "customer"],
    whatsapp_number: ["whatsapp", "phone", "telefono", "tel", "mobile", "celular", "number"],
    email: ["email", "correo", "mail", "e-mail"],
    business_name: ["business", "company", "empresa", "negocio", "businessname"],
    point_of_contact: ["contact", "poc", "pointofcontact", "contacto", "referente"],
...
export function CustomersImport({ tenantId }: { tenantId: string }) {
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const f of IMPORTABLE_CUSTOMER_FIELDS) next[f] = guessColumn(f, parsed.headers);
    setMapping(next);
  }
  ...
  function submit() {
    ...
    const idx: Partial<Record<ImportableCustomerField, number>> = {};
    for (const f of IMPORTABLE_CUSTOMER_FIELDS) {
      const col = mapping[f];
      if (col && col !== IGNORE) idx[f] = headers.indexOf(col);
    }
    const mapped = rows.map((r) => {
      const obj: Partial<Record<ImportableCustomerField, string>> = {};
      for (const f of IMPORTABLE_CUSTOMER_FIELDS) {
        const i = idx[f];
    ...
    startTransition(async () => {
      const res = await importCustomersAction(tenantId, mapped);
      if (!res.ok) {
          ...
          {headers.length > 0 ? (
            <div className="space-y-2">
              ...
              <div className="max-h-64 overflow-y-auto space-y-1.5 pr-1">
                {IMPORTABLE_CUSTOMER_FIELDS.map((f) => (
